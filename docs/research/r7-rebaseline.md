...
| `05b-搜索面板-结果态-light.png` | ⌘K 搜索面板·结果态 | #67 补拍（2026-09-22 23:21）：query `r3 lifecycle probe` → `任务` 组 #1（审核）/#2（已完成）两行；面板几何与 05 全同（520×440 @460,146）。补拍法见 §8 |
...
| 搜索面板结果态（有命中列表） | #67 补拍入库：`05b-搜索面板-结果态-light.png`（§8） | 05b |
...
## 8. 05b 补拍方法与站点漂移（#67，2026-09-22）

- **窗口配方漂移**：§0 的 `1450×829` 窗口在本轮只给 1438×730 视口（浏览器更新后窗框横向 10→12 CSS px、纵向 97→99）；加宽窗口被 Ego 窗管钳回。改用 `Emulation.setDeviceMetricsOverride {1440,732,dpr 2}` 强制视口——仅比物理面大 2px，无 r5/r6 时代的钳制伪影：**核验 = 05b 面板几何与 05 逐值全同**（520×440 @ (460,146)、输入行分隔 y=186、选中行底 `#f1ede7`）。拍毕 `clearDeviceMetricsOverride` 并恢复 maximized。
- **结果态内容**：query `r3 lifecycle probe` → `任务` 组两行（#1 审核 / #2 已完成，双行 row：标题 12px + 项目 10px，右侧时间 11px @x882 + 相位芯片 pill 右距 8）；选 `probe` 短词会额外命中 #46 会话遗留技能 `r3-probe-skill`（`技能` 组），长 query 避开。
- **站点漂移（r7 → 05b 一天内）**：侧栏 `资源` 组新增 `用量` 行（机器 与 模型服务 之间，bar-chart 图标）；r3 遗留 #1 已回回复（`待验收` 列 + `完成` 钮，不再是等待用户态）；#46 会话新增 #11–#14 四条任务。复刻侧 `usageNav` 按场景开关（r7 基线无此行、05b 有）。
- **已完成卡无主按钮**（更正 §3.1 读法）：01b 与 05b 的 `已完成` 列零靛蓝像素——`重开` 只存在于详情 header（§3.3），看板卡不渲染；复刻 `cardAction` 已按此收口。
- **方案▾ 下拉几何**（#67 自 20 位图实测，§3.5/§3.6 未载）：218×44 @ (97,81)——顶边贴 `方案▾` 钮底（y53.5+28）、**右缘对齐钮右缘**（314.5），单行 `方案` + 靛蓝 ✓（右距 4）；radius 10、阴影同弹层族。
