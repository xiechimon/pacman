#!/usr/bin/env bun
/**
 * Build script for standalone Pacman server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for Pacman
 ...
 * not node: builtins, not workspace @pacman/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@pacman/')) continue;
...
function copyWorkspacePackages(config: ServerBuildConfig): void {
  ...
  // PACMAN_MESSAGING_WA_WORKER env resolution in packages/server/src/index.ts.
  const packages = [
    'server',
    'server-core',
    'shared',
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@pacman/server-core/*': ['./packages/server-core/src/*'],
        '@pacman/shared/*': ['./packages/shared/src/*'],
        '@pacman/core/*': ['./packages/core/src/*'],
        '@pacman/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
  writeFileSync(join(outputDir, 'tsconfig.json'), JSON.stringify(rootTsconfig, null, 2) + '\n');

  // Create workspace symlinks in node_modules/@pacman/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@pacman');
    ...
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@pacman/')) {
          const shortName = name.replace('@pacman/', '');
...
export PACMAN_BUNDLED_ASSETS_ROOT="$ROOT"
export PACMAN_IS_PACKAGED=true
export PACMAN_APP_ROOT="$ROOT"
export PACMAN_RESOURCES_PATH="$ROOT/resources"

# CLI tools (doc tools use uv + Python scripts)
export PACMAN_UV="$ROOT/resources/bin/uv"
export PACMAN_SCRIPTS="$ROOT/resources/scripts"
...
if [ -z "\${PACMAN_SERVER_TOKEN:-}" ]; then
  TOKEN=\$(openssl rand -hex 32)
  cat > "$DIR/.env" <<ENVFILE
PACMAN_SERVER_TOKEN=$TOKEN

# TLS — uncomment and set paths to enable wss://
# PACMAN_RPC_TLS_CERT=/path/to/cert.pem
# PACMAN_RPC_TLS_KEY=/path/to/key.pem
# PACMAN_RPC_TLS_CA=/path/to/ca.pem
ENVFILE
  echo ""
  echo "Generated server token (saved to $DIR/.env)"
else
  TOKEN="\$PACMAN_SERVER_TOKEN"
  echo ""
  echo "Using PACMAN_SERVER_TOKEN from environment."
fi
...
echo ""
echo "Quick start:"
echo "  PACMAN_SERVER_TOKEN=$TOKEN $DIR/start.sh"
echo ""
...
ENV PACMAN_BUNDLED_ASSETS_ROOT=/app
ENV PACMAN_APP_ROOT=/app
ENV PACMAN_RESOURCES_PATH=/app/resources
ENV PACMAN_UV=/app/resources/bin/uv
ENV PACMAN_SCRIPTS=/app/resources/scripts
ENV PACMAN_RPC_HOST=0.0.0.0
ENV PACMAN_RPC_PORT=9100
...
services:
  craft-server:
    ...
    ports:
      - "9100:9100"
    environment:
      - PACMAN_SERVER_TOKEN=\${PACMAN_SERVER_TOKEN:?Set PACMAN_SERVER_TOKEN}
      - PACMAN_RPC_PORT=9100
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building Pacman Server ${version} for ${platform}-${arch} ===`);
  ...
  console.log(`\nQuick start:`);
  console.log(`  PACMAN_SERVER_TOKEN=<secret> ${outputDir}/start.sh`);
}
