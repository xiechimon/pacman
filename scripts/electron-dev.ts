...
// Detects instance number from folder name suffix (e.g., pacmans-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.PACMAN_VITE_PORT) return;
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.PACMAN_VITE_PORT || "5173";
