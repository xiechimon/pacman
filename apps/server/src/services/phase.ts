...
// §3.7 新 conv/新 build）、*→closed（右键 Close，r1 §443）、closed→todo
// （reopen，MCP reopen_todos r5 §3.1）。未直接观测的边标 [推断]（04 §3 不判负）。
...
export const PHASE_TRANSITIONS: Readonly<Record<Phase, readonly Phase[]>> = {
  ...
  // 确认→building；驳回→重规划（02 §4.2 确认回路，r5 §4 实走）。
  confirm: ['building', 'planning', 'failed'],
  building: ['review', 'failed'],
  // 完成+合并落地→done（02 §4.2：merge 202 delegated → 机器合并步 → done）。
  review: ['done', 'failed'],
  // done = 终态；reopen 面未观测 [推断]。
  done: [],
