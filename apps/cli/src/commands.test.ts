...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    const prevUrl = process.env.PACMAN_SERVER_URL
    const prevToken = process.env.PACMAN_SERVER_TOKEN
    const prevCa = process.env.PACMAN_TLS_CA

    process.env.PACMAN_SERVER_URL = 'ws://env-server:8080'
    process.env.PACMAN_SERVER_TOKEN = 'env-token'
    process.env.PACMAN_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.PACMAN_SERVER_URL
      else process.env.PACMAN_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.PACMAN_SERVER_TOKEN
      else process.env.PACMAN_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.PACMAN_TLS_CA
      else process.env.PACMAN_TLS_CA = prevCa
    }
  })
