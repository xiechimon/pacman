/**
 * Server spawner — start a headless Pacman server as a child process.
 *
 * Spawns `bun run <serverEntry>`, reads stdout for the `PACMAN_SERVER_URL=`
 * and `PACMAN_SERVER_TOKEN=` lines, and returns a handle to stop the server.
 */
...
export async function spawnServer(opts?: SpawnServerOptions): Promise<SpawnedServer> {
  ...
  // Read stdout line by line looking for PACMAN_SERVER_URL=
  return new Promise<SpawnedServer>((resolve, reject) => {
    ...
    const processLines = () => {
      ...
      for (const line of lines) {
        if (line.startsWith('PACMAN_SERVER_URL=')) {
          url = line.slice('PACMAN_SERVER_URL='.length).trim()
        }
        if (line.startsWith('PACMAN_SERVER_TOKEN=')) {
    ...
    ;(async () => {
      ...
      clearTimeout(timer)
      if (!url) {
        reject(new Error('Server process exited before printing PACMAN_SERVER_URL'))
      }
    })()
  })
}
