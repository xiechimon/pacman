...
export default defineConfig({
  ...
  server: {
    ...
    // end-to-end with HMR. Target port follows PACMAN_RPC_PORT (default 9100).
    // Auto-detects TLS: if the server has PACMAN_RPC_TLS_KEY/CERT set, we proxy
    // over https/wss with secure:false to accept the self-signed dev cert.
    proxy: (() => {
      const port = process.env.PACMAN_RPC_PORT ?? '9100'
      const useTls = Boolean(process.env.PACMAN_RPC_TLS_KEY || process.env.PACMAN_RPC_TLS_CERT)
