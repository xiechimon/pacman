...
import type { RpcServer, RequestContext } from '@pacman/server-core/transport'
...
describe('session file watcher isolation', () => {
  ...
  it('creates independent watchers per client and cleans up on disconnect', async () => {
    ...
    const { registerSessionsHandlers, cleanupSessionFileWatchForClient } = await import('@pacman/server-core/handlers/rpc')
