...
import { redactSensitiveHeadersInPlace, redactSensitiveKeysInPlace } from '@pacman/shared/utils'
...
import { setupI18n, i18n, SUPPORTED_LANGUAGE_CODES, type LanguageCode } from '@pacman/shared/i18n'
import { getPersistedUiLanguage, setPersistedUiLanguage } from '@pacman/shared/config'
setupI18n()
...
import { bootstrapServer, releaseServerLock } from '@pacman/server-core/bootstrap'
...
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@pacman/server-core/model-fetchers'
...
import { initializeBackendHostRuntime } from '@pacman/shared/agent/backend'
import { setPowerShellValidatorRoot } from '@pacman/shared/agent'
...
import { setPerfEnabled, enableDebug } from '@pacman/shared/utils'
import { registerPiModelResolver } from '@pacman/shared/config'
import { getPiModelsForAuthProvider, getAllPiModels } from '@pacman/shared/config'
...
import { validateGitBashPath, checkVCRedistInstalled } from '@pacman/server-core/services'
...
// These are available to all agent Bash sessions via PACMAN_UV, PACMAN_SCRIPTS env vars
// and PATH prepend. uv auto-downloads Python 3.12 on first use (~5s, then cached).
{
  ...
  process.env.PACMAN_IS_PACKAGED = app.isPackaged ? '1' : '0'
  process.env.PACMAN_RESOURCES_BASE = resourcesBase
  process.env.PACMAN_APP_ROOT = app.isPackaged ? app.getAppPath() : process.cwd()

  process.env.PACMAN_UV = bundledUvExists ? uvBinary : (fallbackUv ?? uvBinary)
  ...
  if (existsSync(bunBinary)) {
    process.env.PACMAN_BUN = bunBinary
  }
  ...
  if (!bundledUvExists) {
    mainLog.warn('Bundled uv binary missing, CLI document tools may fail unless uv is available on PATH.', {
      ...
      usingCraftUv: process.env.PACMAN_UV,
    })
  }

  if (isDebugMode) {
    mainLog.info('CLI tools configured:', { uvBinary: process.env.PACMAN_UV, binDir, scriptsDir, bundledUvExists })
  }
}
...
// Only bypasses cert validation for the exact PACMAN_SERVER_URL origin — all other connections
...
if (process.env.PACMAN_SERVER_URL) {
  let serverOrigin: string | undefined
  try {
    serverOrigin = normalizeOriginForCert(process.env.PACMAN_SERVER_URL)
  } catch {
...
app.whenReady().then(async () => {
  ...
  process.env.PACMAN_IS_PACKAGED = app.isPackaged ? 'true' : 'false'
  ...
  // Seed tool icons to ~/.craft/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.craft/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  if (process.platform === 'darwin' && app.dock) {
    ...
    // PACMAN_INSTANCE_NUMBER is set by detect-instance.sh for numbered folders
    const instanceNum = process.env.PACMAN_INSTANCE_NUMBER
    if (instanceNum) {
  ...
  try {
    ...
    // When PACMAN_SERVER_URL is set, this Electron instance is a thin client —
    ...
    const isClientOnly = !!process.env.PACMAN_SERVER_URL
    const isHeadless = !!process.env.PACMAN_HEADLESS

    if (isClientOnly) {
      mainLog.info(`Client-only mode: PACMAN_SERVER_URL=${process.env.PACMAN_SERVER_URL} (server initialization skipped)`)
    }
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        const { getGitBashPath, clearGitBashPath } = await import('@pacman/shared/config')
      ...
      if (process.platform === 'win32') {
        ...
        if (!vcCheck.installed) {
          ...
          process.env.PACMAN_VCREDIST_MISSING = '1'
          if (vcCheck.downloadUrl) {
            process.env.PACMAN_VCREDIST_URL = vcCheck.downloadUrl
          }
        } else if (isDebugMode) {
      ...
      const { getServerConfig } = await import('@pacman/shared/config')
      ...
      const serverToken = serverModeEnabled && embeddedServerConfig.token
        ...
        : randomUUID()
      const rpcHost = process.env.PACMAN_RPC_HOST
        ?? (serverModeEnabled ? '0.0.0.0' : '127.0.0.1')
      const rpcPort = process.env.PACMAN_RPC_PORT
        ? parseInt(process.env.PACMAN_RPC_PORT, 10)
        : (serverModeEnabled ? embeddedServerConfig.port : 0)

      // Load TLS certificates if configured
      let tls: import('@pacman/server-core/transport').WsRpcTlsOptions | undefined
      ...
      const instance = await bootstrapServer<SessionManager, HandlerDeps>({
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            // at ~/.craft/logs/messaging-gateway.log.
        ...
        initModelRefreshService: () => initModelRefreshService(async (slug: string) => {
          const { getCredentialManager } = await import('@pacman/shared/credentials')
      ...
      ipcMain.handle('workspace:remove', async (_event, workspaceId: string) => {
        const { removeWorkspace: remove } = await import('@pacman/shared/config')
      ...
      instance.wsServer.handle(RPC_CHANNELS.settings.GET_SERVER_CONFIG, async () => {
        const { getServerConfig: getConfig } = await import('@pacman/shared/config')
        return getConfig()
      })

      instance.wsServer.handle(RPC_CHANNELS.settings.SET_SERVER_CONFIG, async (_ctx: unknown, config: unknown) => {
        const { setServerConfig: setConfig } = await import('@pacman/shared/config')
        const cfg = config as import('@pacman/shared/config/server-config').ServerConfig
        // Validate port range
...
app.on('window-all-closed', () => {
  if (process.env.PACMAN_HEADLESS) return  // headless server stays alive
