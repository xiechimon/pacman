...
import { DEFAULT_THEME, loadAppTheme, getAllowRemoteEvaluate } from '@pacman/shared/config'
import { CodedError } from '@pacman/shared/protocol'
import { getBrowserLiveFxCornerRadii } from '../shared/browser-live-fx'
import type {
  IBrowserPaneManager,
  BrowserInstanceSnapshot,
} from '@pacman/server-core/handlers'
import type {
...
const PACMAN_DEEPLINK_SCHEME_PREFIX = `${process.env.PACMAN_DEEPLINK_SCHEME || 'craftagents'}://`
...
export class BrowserPaneManager implements IBrowserPaneManager {
  ...
  private buildDeepLinkFromRoute(route: string): string {
    ...
    return `${PACMAN_DEEPLINK_SCHEME_PREFIX}${normalizedPath}${routeQuery ? `?${routeQuery}` : ''}`
  }
  ...
  // validateUploadFilePath removed — uses shared validateFilePath from @pacman/server-core/handlers
...
<html>
  ...
  private async handleDeepLinkUrl(url: string): Promise<void> {
    if (!url.startsWith(PACMAN_DEEPLINK_SCHEME_PREFIX)) return
...
<html>
  ...
  private installThemeObserver(instance: BrowserInstance, allowRetry = true): void {
    ...
    void instance.pageView.webContents.executeJavaScript(`
      (() => {
        ...
        const w = window;
        const previousCleanup = w.__PACMAN_THEME_OBSERVER_CLEANUP__;
        ...
        w.__PACMAN_THEME_OBSERVER_CLEANUP__ = () => {
  ...
  private setupWindowListeners(instance: BrowserInstance): void {
    ...
    pageWc.on('will-navigate', (event, url) => {
      if (url.startsWith(PACMAN_DEEPLINK_SCHEME_PREFIX)) {
        event.preventDefault()
    ...
    pageWc.setWindowOpenHandler((details) => {
      ...
      if (details.url.startsWith(PACMAN_DEEPLINK_SCHEME_PREFIX)) {
