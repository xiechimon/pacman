 ...
 * 2) PACMAN_IS_PACKAGED env (when explicitly set)
...
function resolveDebugMode(): boolean {
  ...
  const packagedEnv = process.env.PACMAN_IS_PACKAGED
 ...
 */
export const messagingGatewayLogPath = join(homedir(), '.pacman', 'logs', 'messaging-gateway.log')
 ...
 */
export const autoUpdateLogPath = join(homedir(), '.pacman', 'logs', 'auto-update.log')
