...
export class WindowManager {
  ...
  private eventSink: ((channel: string, target: import('@pacman/shared/protocol').PushTarget, ...args: any[]) => void) | null = null
   ...
   */
  setRpcEventSink(
    sink: (channel: string, target: import('@pacman/shared/protocol').PushTarget, ...args: any[]) => void,
  ...
  getRpcEventSink(): ((channel: string, target: import('@pacman/shared/protocol').PushTarget, ...args: any[]) => void) | null {
   ...
   *   1 window  → app name ("Pacmans") on the lone window
  ...
  createWindow(options: CreateWindowOptions): BrowserWindow {
    ...
    // The renderer's index.html ships with `<title>Pacmans</title>`, so
