 ...
 * Deep Link Handler
 *
 * Parses pacman:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   pacman://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   pacman://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   pacman://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   pacman://sources[/source/{sourceSlug}]          - Sources list
 *   pacman://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   pacman://action/{actionName}[/{id}][?params]
 *   pacman://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   pacman://allSessions/session/abc123                (specific session)
 *   pacman://settings/shortcuts                     (shortcuts page)
 *   pacman://sources/source/github                  (github source info)
 *   pacman://action/new-chat                        (uses active window)
 *   pacman://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   pacman://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== 'pacman:') {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., pacman://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., pacman://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // pacman://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // pacman://allSessions/..., pacman://settings/..., etc. (compound routes)
    ...
    // pacman://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // pacman://action/... (no workspace - uses active window)
    if (host === 'action') {
