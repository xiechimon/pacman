 ...
 * subprocess with `PACMAN_CONFIG_DIR` set in its env (same pattern as
...
function runScript(configDir: string, script: string): RunResult {
  const result = Bun.spawnSync([process.execPath, '--eval', script], {
    env: { ...process.env, PACMAN_CONFIG_DIR: configDir },
    stdout: 'pipe',
    stderr: 'pipe',
  })
  return {
    ...
    try {
      const r = runScript(
        ...
        `
          ...
          import { setPersistedUiLanguage, getPersistedUiLanguage } from '@pacman/shared/config';
          setupI18n();
  ...
  it('returns undefined when no language is persisted (no hydration step)', () => {
    ...
    try {
      const r = runScript(
        ...
        `
          import { setupI18n, i18n } from '@pacman/shared/i18n';
          import { getPersistedUiLanguage } from '@pacman/shared/config';
          setupI18n();
          const persisted = getPersistedUiLanguage();
