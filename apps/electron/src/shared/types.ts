...
// =============================================================================
export * from '@pacman/shared/protocol'
...
import type {
  ...
  ToolDisplayMeta,
  AnnotationV1,
} from '@pacman/core/types';

// Mode types from dedicated subpath export (avoids pulling in SDK)
import type { PermissionMode } from '@pacman/shared/agent/modes';
export type { PermissionMode };
export { PERMISSION_MODE_CONFIG } from '@pacman/shared/agent/modes';

// Thinking level types
import type { ThinkingLevel } from '@pacman/shared/agent/thinking-levels';
...
import type { AuthState, SetupNeeds } from '@pacman/shared/auth/types';
import type { AuthType } from '@pacman/shared/config/types';
export type { AuthState, SetupNeeds, AuthType };

// Credential health types
import type { CredentialHealthStatus, CredentialHealthIssue, CredentialHealthIssueType } from '@pacman/shared/credentials/types';
...
import type { LoadedSource, FolderSourceConfig, SourceConnectionStatus } from '@pacman/shared/sources/types';
export type { LoadedSource, FolderSourceConfig, SourceConnectionStatus };

// Skill types
import type { LoadedSkill, SkillMetadata } from '@pacman/shared/skills/types';
export type { LoadedSkill, SkillMetadata };

// Resource bundle types (cross-workspace export/import)
import type { ExportResourcesOptions, ExportResult, ResourceImportMode, ResourceBundle, ResourceImportResult } from '@pacman/shared/resources';
export type { ExportResourcesOptions, ExportResult, ResourceImportMode, ResourceBundle, ResourceImportResult };

// LLM connection types
import type { LlmConnection, LlmConnectionWithStatus, LlmAuthType, LlmProviderType, NetworkProxySettings } from '@pacman/shared/config';
...
import type { WorkspaceInfo, Workspace, SessionMetadata, StoredAttachment as StoredAttachmentType } from '@pacman/core/types';
...
export interface ElectronAPI {
  ...
  getServerConfig(): Promise<import('@pacman/shared/config/server-config').ServerConfig>
  setServerConfig(config: import('@pacman/shared/config/server-config').ServerConfig): Promise<void>
  getServerStatus(): Promise<import('@pacman/shared/config/server-config').ServerStatus>
  ...
  // Deep link navigation listener (for external pacman:// URLs)
  ...
  getDraft(sessionId: string): Promise<import('@pacman/shared/config').SessionDraft | null>
  setDraft(sessionId: string, draft: import('@pacman/shared/config').SessionDraft): Promise<void>
  deleteDraft(sessionId: string): Promise<void>
  getAllDrafts(): Promise<Record<string, import('@pacman/shared/config').SessionDraft>>
  ...
  getSourcePermissionsConfig(workspaceId: string, sourceSlug: string): Promise<import('@pacman/shared/agent').PermissionsConfigFile | null>
  getWorkspacePermissionsConfig(workspaceId: string): Promise<import('@pacman/shared/agent').PermissionsConfigFile | null>
  getDefaultPermissionsConfig(): Promise<{ config: import('@pacman/shared/agent').PermissionsConfigFile | null; path: string }>
  ...
  listStatuses(workspaceId: string): Promise<import('@pacman/shared/statuses').StatusConfig[]>
  ...
  listLabels(workspaceId: string): Promise<import('@pacman/shared/labels').LabelConfig[]>
  createLabel(workspaceId: string, input: import('@pacman/shared/labels').CreateLabelInput): Promise<import('@pacman/shared/labels').LabelConfig>
  ...
  listViews(workspaceId: string): Promise<import('@pacman/shared/views').ViewConfig[]>
  saveViews(workspaceId: string, views: import('@pacman/shared/views').ViewConfig[]): Promise<void>
  ...
  createProject(workspaceId: string, input: import('@pacman/shared/projects/types').CreateProjectInput): Promise<import('@pacman/shared/projects/types').ProjectConfig>
  updateProject(workspaceId: string, projectSlug: string, patch: Partial<Omit<import('@pacman/shared/projects/types').ProjectConfig, 'id' | 'slug' | 'createdAt'>>): Promise<import('@pacman/shared/projects/types').ProjectConfig>
  ...
  uploadProjectAsset(workspaceId: string, projectSlug: string, input: { filename: string; base64?: string; text?: string; sourcePath?: string }): Promise<import('@pacman/shared/projects/types').ProjectAsset>
  ...
  getPages(workspaceId: string): Promise<import('@pacman/shared/pages/types').LoadedPage[]>
  getPage(workspaceId: string, pageIdOrSlug: string): Promise<import('@pacman/shared/pages/types').LoadedPage | null>
  createPage(workspaceId: string, input: import('@pacman/shared/pages/types').CreatePageInput): Promise<import('@pacman/shared/pages/types').PageConfig>
  /** Optional fields (projectId, description, refresh) accept explicit null = clear (undefined is dropped by the JSON transport). */
  updatePage(workspaceId: string, pageSlug: string, patch: Partial<Omit<import('@pacman/shared/pages/types').PageConfig, 'id' | 'slug' | 'createdAt' | 'contentDigest' | 'lastRefresh' | 'grants' | 'share' | 'projectId' | 'description' | 'refresh'>> & { projectId?: string | null; description?: string | null; refresh?: import('@pacman/shared/pages/types').PageRefreshSpec | null }): Promise<import('@pacman/shared/pages/types').PageConfig>
  ...
  setPageContent(workspaceId: string, pageSlug: string, content: string): Promise<import('@pacman/shared/pages/types').PageConfig>
  getPageData(workspaceId: string, pageSlug: string): Promise<import('@pacman/shared/pages/types').PageDataSnapshot | null>
  listPageGrants(workspaceId: string, pageSlug: string): Promise<import('@pacman/shared/pages/types').PageActionGrant[]>
  issuePageGrant(workspaceId: string, pageSlug: string, input: { action: import('@pacman/shared/pages/types').PageActionDescriptor; description?: string; ttlMs?: number }): Promise<import('@pacman/shared/pages/types').PageActionGrant>
  revokePageGrant(workspaceId: string, pageSlug: string, grantId: string): Promise<boolean>
  createPageLease(workspaceId: string, pageSlug: string): Promise<{ lease: import('@pacman/shared/pages/types').PageRenderLease; content: string }>
  releasePageLease(workspaceId: string, leaseId: string): Promise<void>
  executePageAction(workspaceId: string, request: import('@pacman/shared/pages/types').PageActionRequest): Promise<import('@pacman/shared/pages/types').PageActionResult>
  ...
  publishPage(workspaceId: string, pageSlug: string, options: { includeData: boolean; password?: string; viewOnlyAcknowledged?: boolean }): Promise<import('@pacman/shared/pages/types').PageConfig>
  setPagePublicationPassword(workspaceId: string, pageSlug: string, password: string | null): Promise<import('@pacman/shared/pages/types').PageConfig>
  unpublishPage(workspaceId: string, pageSlug: string): Promise<{ config: import('@pacman/shared/pages/types').PageConfig; warning?: 'remote-copy-may-remain' }>
  ...
  onPagesChanged(callback: (workspaceId: string, pages: import('@pacman/shared/pages/types').LoadedPage[]) => void): () => void
 ...
 * Mirrors the canonical type in `@pacman/messaging-gateway`.
 */
