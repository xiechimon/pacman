...
import { usePlatform } from '@pacman/ui'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        'The user is editing app-level default permissions (~/.craft/permissions/default.json). ' +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        'The user is editing the notes field in their preferences (~/.craft/preferences.json). ' +
  ...
  'add-source': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.craft/docs/sources.md. ' +
  ...
  'add-source-local': (location) => ({
    context: {
      ...
      context:
        ...
        'First, look up the filesystem source guide in the product docs at #. ' +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.craft/docs/skills.md. ' +
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.craft/docs/labels.md for full format reference. ' +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is tool-icons.json in ~/.craft/tool-icons/. Icon image files live in the same directory. ' +
        ...
        'Read ~/.craft/docs/tool-icons.md for full format reference. ' +
  ...
  'automation-config': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.craft/docs/automations.md for full format reference. ' +
...
export function EditPopover({
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = `pacman://action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`
