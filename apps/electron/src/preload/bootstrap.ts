 ...
 *
 * Thin-client mode (PACMAN_SERVER_URL):
...
import { createCallbackServer } from '@pacman/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@pacman/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@pacman/server-core/transport'
import type { RpcClient } from '@pacman/server-core/transport'
import type { RemoteServerConfig } from '@pacman/core/types'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.PACMAN_SERVER_URL!
  const wsToken = process.env.PACMAN_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set PACMAN_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.PACMAN_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.PACMAN_VCREDIST_MISSING === '1',
  downloadUrl: process.env.PACMAN_VCREDIST_URL,
})
