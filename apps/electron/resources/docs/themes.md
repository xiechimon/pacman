...
This guide explains how to customize the visual theme of Pacman.

## Overview

Pacman uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.pacman/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.pacman/theme.json` - Override specific colors (app-level)
...
```
~/.pacman/workspaces/{id}/config.json
```
...
Create `~/.pacman/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.pacman/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.pacman/themes/{name}.json`
...
1. Create `~/.pacman/theme.json` for overrides or `~/.pacman/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.pacman/theme.json` for overrides, `~/.pacman/themes/` for presets)
