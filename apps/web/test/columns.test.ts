...
import { COLUMNS, sortColumnTodos } from '../src/board/columns.js';
import { todo } from './helpers.js';

const building = COLUMNS.find((c) => c.id === 'building');
...
describe('sortColumnTodos（pinned 组置顶，余者保序）', () => {
  ...
  test('非 pinned 列 = 原序透传', () => {
    const backlog = COLUMNS.find((c) => c.id === 'todo');
    const a = todo(1, 'todo');
  ...
  test('review 非 awaitingReply 在待验收列不置顶（本列无 pinned 组）', () => {
    const reviewCol = COLUMNS.find((c) => c.id === 'review');
