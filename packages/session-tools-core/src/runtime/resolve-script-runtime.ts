...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to PACMAN_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.PACMAN_IS_PACKAGED === '1' || process.env.PACMAN_IS_PACKAGED === 'true';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.PACMAN_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.PACMAN_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute PACMAN_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute PACMAN_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (PACMAN_UV / PACMAN_NODE / PACMAN_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.PACMAN_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.PACMAN_UV, 'Python/uv')
        : process.env.PACMAN_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure PACMAN_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.PACMAN_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.PACMAN_NODE, 'Node')
        : process.env.PACMAN_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure PACMAN_NODE or install node on PATH.'
    );
  }

  if (process.env.PACMAN_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.PACMAN_BUN, 'Bun')
      : process.env.PACMAN_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure PACMAN_BUN or install bun on PATH.'
  );
}
