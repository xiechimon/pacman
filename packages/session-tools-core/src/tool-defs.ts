...
export const UpdatePreferencesSchema = z.object({
  ...
  includeCoAuthoredBy: z.boolean().optional().describe("Whether to include 'Co-Authored-By: Pacman' trailer on git commits. Defaults to true."),
});
...
export const CreatePageSchema = z.object({
  ...
  content: z.string().optional().describe('Full self-contained HTML document for index.html (inline CSS/JS, no external requests). Read ~/.pacman/docs/pages.md for the authoring guide and data-bridge snippet BEFORE writing page HTML.'),
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
  ...
  config_validate: `Validate Pacman configuration files.
...
Requires an explicit sessionId and cannot target your own session. Use list_sessions / get_session_info to find the target session's ID.`,
  ...
  create_task: `Create a Pacmans Task on the kanban board — writes tasks/<slug>/task.yaml and creates its orchestrator session. CREATION ONLY: the task lands in "todo" and is NOT run; starting it is the user's (or an automation's) decision.
...
IMPORTANT — read ~/.pacman/docs/pages.md BEFORE authoring page HTML. Key rules: provide a FULL standalone HTML document with ALL CSS/JS inline (no external requests — shared copies get network egress blocked); to display data from the page's data store, listen for the 'craft-pages/v1' bridge messages (init/data) documented there; kind 'live' pages receive replacement data snapshots automatically while open.
