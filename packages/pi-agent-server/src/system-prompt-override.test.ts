...
/**
 * Regression contract for pacmans-oss#648.
 *
...
describe('applySystemPromptOverride', () => {
  it('stamps state.systemPrompt', () => {
    ...
    applySystemPromptOverride(session as unknown as AgentSession, 'PACMAN_PROMPT');
    expect(session.agent.state.systemPrompt).toBe('PACMAN_PROMPT');
  });

  it('stamps the private _baseSystemPrompt field so session.prompt() reset survives', () => {
    const session = makeFakeSession();
    applySystemPromptOverride(session as unknown as AgentSession, 'PACMAN_PROMPT');
    expect(session._baseSystemPrompt).toBe('PACMAN_PROMPT');
  });
  ...
  it('replaces _rebuildSystemPrompt with a constant function so tool-change rebuilds survive', () => {
    ...
    expect(session._rebuildSystemPrompt!(['read', 'bash'])).toBe('PACMAN_PROMPT');
    expect(session._rebuildSystemPrompt!([])).toBe('PACMAN_PROMPT');
  });
