...
export class Commands {
  ...
  private async handlePair(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    if (!/^\d{6}$/.test(code)) {
      await adapter.sendText(
        msg.channelId,
        'Usage: /pair <6-digit code>\n\nGenerate a code from the session menu or the Telegram supergroup setup in the Pacman app.',
        replyOpts,
      )
      return
    }
    ...
    if (
      ...
      await adapter.sendText(
        msg.channelId,
        'Only existing bot owners can redeem pairing codes. Ask an owner to add you in the Pacman app.',
        replyOpts,
      )
      return
    }
