...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.craft/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
