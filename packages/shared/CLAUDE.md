# CLAUDE.md — `@pacman/shared`

## Purpose
Core business logic package for Pacman:
...
- WebUI source OAuth does not use a hosted relay in pacman — the relay callback URL is empty by default (`OAUTH_RELAY_CALLBACK_URL = ''`), so any attempt to relay Slack/Google/Microsoft OAuth will fail fast at the provider. Sources that need a hosted relay must inject one via `OAUTH_RELAY_CALLBACK_URL` at build time.
...
3. **Keep brand names in English**: Pacman, Workspace, Claude, Anthropic, OpenAI, MCP, API, SDK.
...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.pacman/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
