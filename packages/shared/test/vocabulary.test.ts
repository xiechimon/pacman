...
describe('record projection (01 §6 / 03 M1; M4a +chief)', () => {
  it('DB table registry is the 01 §6 list + chief + steer/stop_pending (28 incl. the todo_tag join)', () => {
    expect(DB_TABLES).toHaveLength(28);
    expect(DB_TABLES).toContain('todo_tag');
    expect(DB_TABLES).toContain('chief');
  });

  it('record shapes cover exactly the 25 wire tables (todo_tag join + steer/stop_pending internal have none)', () => {
