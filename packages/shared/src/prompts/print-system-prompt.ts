#!/usr/bin/env bun
/**
 * Debug script to print the full Pacman system prompt with annotations.
...
printAnnotation('  2. Pacman Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.craft/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@pacman/electron/main.log' },
  '/Users/example/.craft/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.craft/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by Pacman.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Pacman additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Pacman Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/pacman.ts       ${colors.dim}// User message building${colors.reset}
