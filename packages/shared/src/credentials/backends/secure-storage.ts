 ...
 *
 * Stores credentials in an encrypted file at ~/.pacman/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.pacman');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('pacman-v2') // Bumped version for new key derivation
      .digest();
