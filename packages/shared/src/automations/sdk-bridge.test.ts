...
describe('sdk-bridge', () => {
  describe('buildEnvFromSdkInput', () => {
    it('should always include PACMAN_EVENT', () => {
      const env = buildEnvFromSdkInput('PreToolUse', input());
      expect(env.PACMAN_EVENT).toBe('PreToolUse');
    });
    ...
    describe('PreToolUse / PostToolUse', () => {
      it('should map tool_name to PACMAN_TOOL_NAME', () => {
        const env = buildEnvFromSdkInput('PreToolUse', input({ tool_name: 'Bash' }));
        expect(env.PACMAN_TOOL_NAME).toBe('Bash');
      });
      ...
      it('should map tool_input as sanitized JSON', () => {
        ...
        expect(env.PACMAN_TOOL_INPUT).toBeDefined();
        expect(env.PACMAN_TOOL_INPUT).not.toContain('`');
      });
      ...
      it('should map tool_response for PostToolUse', () => {
        ...
        expect(env.PACMAN_TOOL_RESPONSE).toBeDefined();
      });
    });

    describe('PostToolUseFailure', () => {
      it('should map error to PACMAN_ERROR', () => {
        const env = buildEnvFromSdkInput('PostToolUseFailure', input({
          tool_name: 'Bash',
          error: 'Command failed',
        }));
        expect(env.PACMAN_TOOL_NAME).toBe('Bash');
        expect(env.PACMAN_ERROR).toBeDefined();
      });
    });
    ...
    describe('UserPromptSubmit', () => {
      it('should sanitize user prompt', () => {
        ...
        expect(env.PACMAN_PROMPT).toBeDefined();
        // Backticks should be escaped with backslash
        expect(env.PACMAN_PROMPT).toContain('\\`');
      });
    });
    ...
    describe('SessionStart', () => {
      it('should map source and model', () => {
        ...
        expect(env.PACMAN_SOURCE).toBe('manual');
        expect(env.PACMAN_MODEL).toBe('claude-opus-4-7');
      });
    });
    ...
    describe('SubagentStart / SubagentStop', () => {
      it('should map agent_id and agent_type', () => {
        ...
        expect(env.PACMAN_AGENT_ID).toBe('agent-123');
        expect(env.PACMAN_AGENT_TYPE).toBe('research');
      });
    });
    ...
    describe('Notification', () => {
      it('should sanitize message and title', () => {
        ...
        expect(env.PACMAN_MESSAGE).toBeDefined();
        expect(env.PACMAN_TITLE).toBeDefined();
        // Backticks should be escaped
        expect(env.PACMAN_MESSAGE).toContain('\\`');
        expect(env.PACMAN_TITLE).toContain('\\`');
      });
    });
    ...
    describe('unknown/default events', () => {
      it('should return minimal env for events with no specific mappings', () => {
        ...
        expect(env.PACMAN_EVENT).toBe('Stop');
