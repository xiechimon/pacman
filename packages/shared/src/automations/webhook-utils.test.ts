...
const env = {
  PACMAN_WH_SESSION_ID: 'sess-123',
  PACMAN_WH_EVENT: 'LabelAdd',
...
describe('expandWebhookAction', () => {
  it('expands URL templates', () => {
    const action: WebhookAction = {
      type: 'webhook',
      url: 'https://api.example.com/hook/${PACMAN_WH_SESSION_ID}',
    };
  ...
  it('expands header values', () => {
    const action: WebhookAction = {
      ...
      headers: { 'X-Event': '${PACMAN_WH_EVENT}', 'X-Static': 'unchanged' },
    };
    const result = expandWebhookAction(action, env);
  ...
  it('expands string body', () => {
    const action: WebhookAction = {
      type: 'webhook',
      url: 'https://api.example.com',
      body: 'session=${PACMAN_WH_SESSION_ID}',
      bodyFormat: 'raw',
    };
    const result = expandWebhookAction(action, env);
  ...
  it('expands object body (JSON)', () => {
    const action: WebhookAction = {
      type: 'webhook',
      url: 'https://api.example.com',
      body: { id: '${PACMAN_WH_SESSION_ID}', event: '${PACMAN_WH_EVENT}' },
    };
    const result = expandWebhookAction(action, env);
  ...
  it('expands basic auth credentials', () => {
    const action: WebhookAction = {
      type: 'webhook',
      url: 'https://api.example.com',
      auth: { type: 'basic', username: '${PACMAN_WH_SESSION_ID}', password: '${API_TOKEN}' },
    };
    const result = expandWebhookAction(action, env);
