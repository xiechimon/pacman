  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.pacman/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files (~/.pacman/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.pacman/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.pacman/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.pacman/tool-icons/tool-icons.json → tool icon mappings
 */
