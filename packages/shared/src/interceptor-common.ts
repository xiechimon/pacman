...
export const DEBUG = INTERCEPTOR_LOGGING_ENABLED &&
  (process.argv.includes('--debug') || process.env.PACMAN_DEBUG === '1');

/** Config file path for reading settings in the SDK subprocess */
export const CONFIG_FILE = join(homedir(), '.pacman', 'config.json');

/** Session directory — set by env var (subprocess) or setSessionDir() (main process) */
let _sessionDir: string | null = process.env.PACMAN_SESSION_DIR || null;

// ============================================================================
// LOGGING
// ============================================================================

export const LOG_DIR = join(homedir(), '.pacman', 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(homedir(), '.pacman', 'api-error.json');
}
 ...
 * - SDK subprocess: PACMAN_SESSION_DIR env var (set by main process before spawn)
