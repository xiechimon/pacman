 ...
 * Default location: ~/.pacman/workspaces/
 */
...
const CONFIG_DIR = join(homedir(), '.pacman');
...
/**
 * Get the default workspaces directory (~/.pacman/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.pacman/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.pacman/workspaces/
 */
 ...
 * Resolution order: ENV (PACMAN_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.PACMAN_LOCAL_MCP_ENABLED;
