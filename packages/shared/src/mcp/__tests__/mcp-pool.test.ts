...
describe('McpClientPool.ensureConnected', () => {
  ...
  test('refuses stdio configs when local MCP is disabled for the workspace', async () => {
    const prev = process.env.PACMAN_LOCAL_MCP_ENABLED;
    process.env.PACMAN_LOCAL_MCP_ENABLED = 'false';
    try {
      ...
      if (prev === undefined) delete process.env.PACMAN_LOCAL_MCP_ENABLED;
      else process.env.PACMAN_LOCAL_MCP_ENABLED = prev;
    }
  });
});
