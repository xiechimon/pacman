...
describe('resolveKeepBackgroundTasksAlive', () => {
  ...
  it('is ON for "1"/"true"', () => {
    expect(resolveKeepBackgroundTasksAlive({ PACMAN_KEEP_BG_AGENTS_ALIVE: '1' })).toBe(true);
    expect(resolveKeepBackgroundTasksAlive({ PACMAN_KEEP_BG_AGENTS_ALIVE: 'true' })).toBe(true);
  });
  it('is OFF for "0"/"false" (explicit kill-switch)', () => {
    expect(resolveKeepBackgroundTasksAlive({ PACMAN_KEEP_BG_AGENTS_ALIVE: '0' })).toBe(false);
    expect(resolveKeepBackgroundTasksAlive({ PACMAN_KEEP_BG_AGENTS_ALIVE: 'false' })).toBe(false);
  });
});
