...
// Plan types are used by UI components; not needed in pacman.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.craft/docs/
...
export class ClaudeAgent extends BaseAgent {
   ...
   * `PACMAN_KEEP_BG_AGENTS_ALIVE` via the shared resolver. Default is ON (opt-out);
   * set `PACMAN_KEEP_BG_AGENTS_ALIVE=0` to force the per-turn kill-switch path.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      // Gated by enable1MContext in global config (~/.craft/config.json).
      ...
      const options: Options = {
        ...
        hooks: (() => {
          ...
          if (Object.keys(userHooks).length > 0) {
            debug('[Pacman] User SDK hooks loaded:', Object.keys(userHooks).join(', '));
          }
...
// ============================================================
// These aliases allow gradual migration from Pacman to ClaudeAgent.
// Once all consumers are updated, these can be removed.

/** @deprecated Use ClaudeAgent instead */
export { ClaudeAgent as Pacman };

/** @deprecated Use ClaudeAgentConfig instead */
export type { ClaudeAgentConfig as PacmanConfig };
