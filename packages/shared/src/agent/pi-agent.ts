...
// System prompt for Pacman context
  ...
  switch (code) {
    ...
    case 'CAPABILITY_UNAVAILABLE':
      return 'No connected desktop client supports browser tools, or no client is currently connected. ' +
        'Ask the user to open this workspace from the Pacman desktop app.';
...
export class PiAgent extends BaseAgent {
  ...
  // Ring buffer of recent subprocess stderr. Always on (independent of PACMAN_DEBUG)
  ...
  private async spawnSubprocess(): Promise<void> {
    ...
    const child = spawn(nodePath, args, {
      ...
      env: {
        ...
        ...(sessionDir ? { PACMAN_SESSION_DIR: sessionDir } : {}),
    ...
    // Keep the PACMAN_DEBUG-gated log for interactive dev work.
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to PacmanEvents.
   ...
   * Uses the canonical registry from @pacman/session-tools-core.
   */
