 ...
 *
 * File locations:
 * - Workspace: ~/.pacman/workspaces/{slug}/permissions.json
 * - Per-source: ~/.pacman/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.pacman/permissions/
 * Reads env var dynamically so tests can override via PACMAN_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.PACMAN_CONFIG_DIR || join(homedir(), '.pacman');
...
/**
 * Load default permissions from ~/.pacman/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.pacman/permissions/default.json)
   ...
   * These come from ~/.pacman/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping pacman bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
