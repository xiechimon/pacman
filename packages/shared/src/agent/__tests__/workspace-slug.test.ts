...
describe('workspace slug extraction', () => {
  ...
  it('handles Windows-style paths with backslashes', () => {
    expect(extractWorkspaceSlug('C:\\Users\\ghalmos\\.pacman\\workspaces\\my-workspace', fallback)).toBe('my-workspace')
  })

  it('handles Windows paths with tilde and backslashes', () => {
    expect(extractWorkspaceSlug('~\\.pacman\\workspaces\\my-workspace', fallback)).toBe('my-workspace')
  })
