...
describe('createBrowserTools', () => {
  ...
  describe('browser_tool', () => {
    ...
    it('uses focus fallback when foreground open visibility does not settle in wait loop', async () => {
      const previousTimeout = process.env.PACMAN_BROWSER_OPEN_SETTLE_TIMEOUT_MS
      const previousPoll = process.env.PACMAN_BROWSER_OPEN_SETTLE_POLL_MS
      process.env.PACMAN_BROWSER_OPEN_SETTLE_TIMEOUT_MS = '120'
      process.env.PACMAN_BROWSER_OPEN_SETTLE_POLL_MS = '20'
      ...
      try {
        ...
        if (previousTimeout === undefined) delete process.env.PACMAN_BROWSER_OPEN_SETTLE_TIMEOUT_MS
        else process.env.PACMAN_BROWSER_OPEN_SETTLE_TIMEOUT_MS = previousTimeout

        if (previousPoll === undefined) delete process.env.PACMAN_BROWSER_OPEN_SETTLE_POLL_MS
        else process.env.PACMAN_BROWSER_OPEN_SETTLE_POLL_MS = previousPoll
      }
    })
