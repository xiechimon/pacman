...
import {
  CLI_DOMAIN_POLICIES,
  PACMAN_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES,
  PACMAN_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES,
...
export function qualifySkillName(
  ...
  // Extract the bare slug — strip any existing qualifier (e.g. "PacmanWS:commit" → "commit")
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = PACMAN_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use pacman commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^pacman\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = PACMAN_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES
...
export function runPreToolUseChecks(ctx: PreToolUseInput): PreToolUseCheckResult {
  ...
  // 5b. Config-domain Bash guard (block direct labels/automations path operations unless using pacman)
  if (FEATURE_FLAGS.pacmanCli && toolName === 'Bash') {
