 ...
 *
 * The `ErrorCode` union is owned by `@pacman/core` so the wire
...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for Pacman. Please choose a model with tool support (e.g., Claude, GPT-4, Gemini).',
    actions: [
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for Pacman. Please choose a different model with tool support in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
