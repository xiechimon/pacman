...
import type { PageConfig, PageShareInfo } from '@pacman/core';
...
/**
 * Resolve the publication API base URL. `PACMAN_PAGES_SHARE_API_URL` overrides
 * for local Worker development (e.g. http://localhost:8787/p/api).
 */
export function resolvePagesShareApiBaseUrl(): string {
  const override =
    typeof process !== 'undefined' ? process.env?.PACMAN_PAGES_SHARE_API_URL : undefined;
  ...
  private assertSharingEnabled(): void {
    if (!isPagesSharingEnabled()) {
      throw new PageShareError(
        ...
        'Pages sharing is disabled (set PACMAN_FEATURE_PAGES_SHARING=1 to enable)',
      );
    }
  }
