#!/usr/bin/env bun
/**
 * @pacman/server — standalone headless Pacman server.
 *
 * Usage:
 *   PACMAN_SERVER_TOKEN=<secret> bun run packages/server/src/index.ts
 *
 * Environment:
 *   PACMAN_SERVER_TOKEN         — required bearer token for client auth
 *   PACMAN_RPC_HOST             — bind address (default: 127.0.0.1)
 *   PACMAN_RPC_PORT             — bind port (default: 9100)
 *   PACMAN_RPC_TLS_CERT         — path to PEM certificate file (enables TLS/wss)
 *   PACMAN_RPC_TLS_KEY          — path to PEM private key file (required with cert)
 *   PACMAN_RPC_TLS_CA           — path to PEM CA chain file (optional)
 *   PACMAN_APP_ROOT             — app root path (default: cwd)
 *   PACMAN_RESOURCES_PATH       — resources path (default: cwd/resources)
 *   PACMAN_IS_PACKAGED          — 'true' for production (default: false)
 *   PACMAN_VERSION              — app version (default: 0.0.0-dev)
 *   PACMAN_DEBUG                — 'true' for debug logging
 *   PACMAN_WEBUI_DIR            — path to built web UI assets (enables web UI on RPC port)
 *   PACMAN_WEBUI_PASSWORD       — optional shorter password for web login (falls back to PACMAN_SERVER_TOKEN)
 *   PACMAN_WEBUI_SECURE_COOKIE  — optional true/false override for the session cookie Secure flag
 *   PACMAN_WEBUI_WS_URL         — optional browser-facing ws:// or wss:// URL returned by /api/config
 *   PACMAN_MESSAGING_WA_WORKER  — absolute path to worker.cjs (default: packages/messaging-whatsapp-worker/dist/worker.cjs)
 *   PACMAN_MESSAGING_NODE_BIN   — Node binary used to spawn the WhatsApp worker (default: node)
 */
...
import { enableDebug } from '@pacman/shared/utils/debug'
import { bootstrapServer, startHealthHttpServer, generateServerToken } from '@pacman/server-core/bootstrap'
import { validateSession, createWebuiHandler, nodeHttpAdapter } from '@pacman/server-core/webui'
import type { WebuiHandler } from '@pacman/server-core/webui'
import { getCredentialManager } from '@pacman/shared/credentials'
import { getWorkspaces } from '@pacman/shared/config'
import { createMessagingBootstrap, type MessagingBootstrapHandle } from '@pacman/messaging-gateway'
...
import type { WsRpcTlsOptions } from '@pacman/server-core/transport'
import { registerCoreRpcHandlers, cleanupSessionFileWatchForClient } from '@pacman/server-core/handlers/rpc'
import { SessionManager, setSessionPlatform, setSessionRuntimeHooks } from '@pacman/server-core/sessions'
import { initModelRefreshService, setFetcherPlatform } from '@pacman/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@pacman/server-core/services'
import type { HandlerDeps } from '@pacman/server-core/handlers'

process.env.PACMAN_IS_PACKAGED ??= 'false'
...
if (process.env.PACMAN_DEBUG === 'true' || process.env.PACMAN_DEBUG === '1') {
  enableDebug()
}
...
// In packaged mode, use PACMAN_BUNDLED_ASSETS_ROOT env or cwd.
const bundledAssetsRoot = process.env.PACMAN_BUNDLED_ASSETS_ROOT
  ?? join(import.meta.dir, '..', '..', '..', '..')

// TLS configuration — when cert + key paths are provided, server listens on wss://
let tls: WsRpcTlsOptions | undefined
const tlsCertPath = process.env.PACMAN_RPC_TLS_CERT
const tlsKeyPath = process.env.PACMAN_RPC_TLS_KEY
if (tlsCertPath || tlsKeyPath) {
  if (!tlsCertPath || !tlsKeyPath) {
    console.error('TLS requires both PACMAN_RPC_TLS_CERT and PACMAN_RPC_TLS_KEY.')
    process.exit(1)
  }
  tls = {
    cert: readFileSync(tlsCertPath),
    key: readFileSync(tlsKeyPath),
    ...(process.env.PACMAN_RPC_TLS_CA ? { ca: readFileSync(process.env.PACMAN_RPC_TLS_CA) } : {}),
  }
}

// Web UI configuration
const webuiDir = process.env.PACMAN_WEBUI_DIR || undefined
const webuiEnabled = webuiDir && existsSync(webuiDir)
const webuiSecureCookies = parseOptionalBooleanEnv('PACMAN_WEBUI_SECURE_COOKIE', process.env.PACMAN_WEBUI_SECURE_COOKIE)
const webuiWsUrl = parseOptionalWebSocketUrl('PACMAN_WEBUI_WS_URL', process.env.PACMAN_WEBUI_WS_URL)
const serverToken = process.env.PACMAN_SERVER_TOKEN
...
if (webuiEnabled && serverToken) {
  const rpcPort = parseInt(process.env.PACMAN_RPC_PORT ?? '9100', 10)
  const rpcProtocol = tls ? 'wss' as const : 'ws' as const

  webuiHandler = createWebuiHandler({
    webuiDir: webuiDir!,
    secret: serverToken,
    password: process.env.PACMAN_WEBUI_PASSWORD || undefined,
...
const waWorkerEntry = process.env.PACMAN_MESSAGING_WA_WORKER
  ?? join(bundledAssetsRoot, 'packages', 'messaging-whatsapp-worker', 'dist', 'worker.cjs')
const waNodeBin = process.env.PACMAN_MESSAGING_NODE_BIN ?? 'node'
...
const instance = await (async () => {
  try {
    return await bootstrapServer<SessionManager, HandlerDeps>({
      bundledAssetsRoot,
      serverVersion: process.env.PACMAN_VERSION ?? packageVersion,
      tls,
      ...
      createHandlerDeps: ({ sessionManager, platform, oauthFlowStore }) => {
        messagingHandle = createMessagingBootstrap({
          ...
          getMessagingDir: (wsId: string) =>
            join(homedir(), '.pacman', 'workspaces', wsId, 'messaging'),
...
// ---------------------------------------------------------------------------
// PACMAN_DISABLE_MESSAGING lets a dev/test server share a config dir with a live app
// without both processes fighting over the same Telegram/WhatsApp connections (409s).
const messagingDisabled = process.env.PACMAN_DISABLE_MESSAGING === 'true' || process.env.PACMAN_DISABLE_MESSAGING === '1'
if (messagingHandle !== null && !messagingDisabled) {
  ...
  console.log('[messaging] Disabled via PACMAN_DISABLE_MESSAGING — skipping workspace messaging init')
}

// Wire up the lazy health check now that the session manager is ready
if (webuiHandler) {
  const { getHealthCheck } = await import('@pacman/server-core/handlers/rpc/server')
  ...
  const { getSourceCredentialManager, loadWorkspaceSources } = await import('@pacman/shared/sources')
  const { getWorkspaceByNameOrId } = await import('@pacman/shared/config')
  const { pushTyped } = await import('@pacman/server-core/transport')
  const { RPC_CHANNELS } = await import('@pacman/shared/protocol')
...
// Start HTTP health endpoint if PACMAN_HEALTH_PORT is set
const healthPort = parseInt(process.env.PACMAN_HEALTH_PORT ?? '0', 10)
...
console.log(`PACMAN_SERVER_URL=${instance.protocol}://${instance.host}:${instance.port}`)
console.log(`PACMAN_SERVER_TOKEN=${instance.token}`)
if (webuiHandler) {
  console.log(`PACMAN_WEBUI_URL=${serverProto}://0.0.0.0:${instance.port}`)
}
...
if (!isLocalBind && instance.protocol === 'ws') {
  if (process.argv.includes('--allow-insecure-bind')) {
    console.warn(
      ...
      '   Set PACMAN_RPC_TLS_CERT and PACMAN_RPC_TLS_KEY to enable wss://.\n'
    )
  } else {
    console.error(
      ...
      '   Options:\n' +
      '     1. Set PACMAN_RPC_TLS_CERT and PACMAN_RPC_TLS_KEY to enable wss://\n' +
