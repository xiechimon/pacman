...
import type { StoredAttachment, ContentBadge } from '@pacman/core'
import { normalizePath } from '@pacman/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .pacman workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.pacman\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
