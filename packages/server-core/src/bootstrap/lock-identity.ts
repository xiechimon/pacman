 ...
 *   as ours only when its command line references the app (current `pacman`
 *   branding, or `craft` in locks written by pre-rename builds).
...
export function lockHolderMatchesLock(
  ...
  return /craft|pacman/i.test(liveCommandLine)
}
