...
describe('parseTasklistImageName', () => {
  it('extracts the image name from a CSV data row', () => {
    expect(parseTasklistImageName('"Pacmans.exe","1234","Console","1","150,000 K"')).toBe('Pacmans.exe')
  })
...
describe('lockHolderMatchesLock (#978)', () => {
  ...
  it('matches on identical execName', () => {
    expect(lockHolderMatchesLock(lockWith('Pacmans.exe'), 'Pacmans.exe', null)).toBe(true)
  })

  it('matches execName case-insensitively', () => {
    expect(lockHolderMatchesLock(lockWith('craft agents.exe'), 'Pacmans.EXE', null)).toBe(true)
  })
  ...
  it('rejects a different executable even when its name contains "craft"', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Pacmans.exe'), 'minecraft-launcher', null)).toBe(false)
  })

  it('fails open when the live exec name cannot be determined', () => {
    expect(lockHolderMatchesLock(lockWith('Pacmans.exe'), null, null)).toBe(false)
  })
  ...
  it('ignores the command line when execName is recorded', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Pacmans.exe'), 'java', '/usr/bin/java -jar minecraft.jar')).toBe(false)
  })

  describe('legacy locks without execName', () => {
    it('falls back to the craft-substring heuristic on the command line', () => {
      expect(lockHolderMatchesLock(lockWith(undefined), null, '/Applications/Pacmans.app/Contents/MacOS/Pacmans')).toBe(true)
