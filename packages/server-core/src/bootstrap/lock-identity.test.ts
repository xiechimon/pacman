...
describe('parseTasklistImageName', () => {
  it('extracts the image name from a CSV data row', () => {
    expect(parseTasklistImageName('"Pacman.exe","1234","Console","1","150,000 K"')).toBe('Pacman.exe')
  })
...
describe('lockHolderMatchesLock (#978)', () => {
  ...
  it('matches on identical execName', () => {
    expect(lockHolderMatchesLock(lockWith('Pacman.exe'), 'Pacman.exe', null)).toBe(true)
  })
  ...
  it('rejects a different executable even when its name contains "craft"', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Pacman.exe'), 'minecraft-launcher', null)).toBe(false)
  })

  it('fails open when the live exec name cannot be determined', () => {
    expect(lockHolderMatchesLock(lockWith('Pacman.exe'), null, null)).toBe(false)
  })
  ...
  it('ignores the command line when execName is recorded', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Pacman.exe'), 'java', '/usr/bin/java -jar minecraft.jar')).toBe(false)
  })

  describe('legacy locks without execName', () => {
    it('falls back to the craft-substring heuristic on the command line', () => {
      expect(lockHolderMatchesLock(lockWith(undefined), null, '/Applications/Pacman.app/Contents/MacOS/Pacman')).toBe(true)
