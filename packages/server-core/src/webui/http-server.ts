...
import { generateCallbackPage } from '@pacman/shared/auth'
...
export interface WebuiHandlerOptions {
  ...
  webuiDir: string
  /** Secret used to sign JWTs — typically PACMAN_SERVER_TOKEN. */
  secret: string
...
export function createWebuiHandler(options: WebuiHandlerOptions): WebuiHandler {
  ...
  async function fetch(req: Request): Promise<Response> {
    ...
    if (path === '/api/config/workspaces' && req.method === 'GET') {
      ...
      const { getActiveWorkspace } = await import('@pacman/shared/config/storage')
