...
export function parseTestConnectionError(msg: string): string {
  ...
  if (lower.includes('no api key found for')) {
    return 'Provider mismatch during setup. Select a provider preset in Pacman Backend API Key mode, or use Anthropic API Key mode for arbitrary compatible endpoints.'
  }
...
export function validateSetupTestInput(params: {
  ...
  if (params.provider === 'pi' && hasCustomEndpoint && !params.piAuthProvider) {
    return {
      valid: false,
      error: 'Custom endpoint in Pacman Backend mode requires selecting a provider preset. For arbitrary Anthropic-compatible endpoints, use Anthropic API Key mode.',
    }
  }
