...
import { appendAutomationHistoryEntry } from '@pacman/shared/automations/history-store'
import { AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER } from '@pacman/shared/automations/constants'
import type { RpcServer } from '@pacman/server-core/transport'
import type { HandlerDeps } from '../handler-deps'

// History file name — matches AUTOMATIONS_HISTORY_FILE from @pacman/shared/automations/constants
...
async function withAutomationMatcher(workspaceId: string, eventName: string, matcherIndex: number, mutate: (matchers: Record<string, unknown>[], index: number, config: AutomationsConfigJson, genId: () => string) => void) {
  ...
  await withConfigMutex(workspace.rootPath, async () => {
    const { resolveAutomationsConfigPath, generateShortId } = await import('@pacman/shared/automations/resolve-config-path')
...
export function registerAutomationsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.automations.GET, async (_ctx, workspaceId: string) => {
    ...
    try {
      const { resolveAutomationsConfigPath } = await import('@pacman/shared/automations/resolve-config-path')
      const configPath = resolveAutomationsConfigPath(workspace.rootPath)
  ...
  server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload: import('@pacman/shared/protocol').TestAutomationPayload) => {
    const workspace = getWorkspaceByNameOrId(payload.workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const results: import('@pacman/shared/protocol').TestAutomationActionResult[] = []
    const { parsePromptReferences } = await import('@pacman/shared/automations')
    const { executeWebhookRequest, createWebhookHistoryEntry, createPromptHistoryEntry } = await import('@pacman/shared/automations/webhook-utils')
    ...
    for (const action of payload.actions) {
      ...
      if (action.type === 'webhook') {
        ...
        const result = await executeWebhookRequest(action as import('@pacman/shared/automations').WebhookAction)
      ...
      if (action.type === 'script') {
        ...
        const { executeScriptAction, createScriptHistoryEntry, buildScriptEnv } = await import('@pacman/shared/automations')
      ...
      try {
        const { sessionId } = await deps.sessionManager.executePromptAutomation({
          ...
          // trip the 30s RPC timeout (pacmans-oss#943).
    ...
    return { actions: results } satisfies import('@pacman/shared/protocol').TestAutomationResult
  })
  ...
  server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId: string, automationId: string, eventName: string) => {
    ...
    const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@pacman/shared/automations/webhook-utils')
    const results = await Promise.all(
      webhookActions.map(a => executeWebhookRequest(a as unknown as import('@pacman/shared/automations').WebhookAction))
    )
