...
export function registerLabelsHandlers(server: RpcServer, _deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.labels.LIST, async (_ctx, workspaceId: string) => {
    ...
    const { listLabels } = await import('@pacman/shared/labels/storage')
    return listLabels(workspace.rootPath)
  })

  // Create a new label in a workspace
  server.handle(RPC_CHANNELS.labels.CREATE, async (_ctx, workspaceId: string, input: import('@pacman/shared/labels').CreateLabelInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { createLabel } = await import('@pacman/shared/labels/crud')
  ...
  server.handle(RPC_CHANNELS.labels.DELETE, async (_ctx, workspaceId: string, labelId: string) => {
    ...
    const { deleteLabel } = await import('@pacman/shared/labels/crud')
