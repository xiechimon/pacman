...
import { RPC_CHANNELS, type FileAttachment, type SendMessageOptions, type SessionEvent } from '@pacman/shared/protocol'
import type { StoredAttachment } from '@pacman/core/types'
import { getWorkspaceByNameOrId } from '@pacman/shared/config'
import { perf } from '@pacman/shared/utils'
import { isValidThinkingLevel, THINKING_LEVEL_IDS } from '@pacman/shared/agent/thinking-levels'
...
async function scanSessionDirectory(dirPath: string): Promise<import('@pacman/shared/protocol').SessionFile[]> {
  const { readdir, stat } = await import('fs/promises')
  const entries = await readdir(dirPath, { withFileTypes: true })
  const files: import('@pacman/shared/protocol').SessionFile[] = []
...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId: string, options?: import('@pacman/shared/protocol').CreateSessionOptions) => {
  ...
  // Returns true if the response was delivered, false if agent/session is gone
  server.handle(RPC_CHANNELS.sessions.RESPOND_TO_CREDENTIAL, async (_ctx, sessionId: string, requestId: string, response: import('@pacman/shared/protocol').CredentialResponse) => {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    _ctx,
    sessionId: string,
    command: import('@pacman/shared/protocol').SessionCommand
  ) => {
    switch (command.type) {
      case 'flag':
  ...
  server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { searchSessions } = await import('@pacman/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@pacman/shared/workspaces')
  ...
  const importHandler = async (_ctx: any, targetWorkspaceId: string, bundle: unknown, mode: string) => {
    ...
    return sessionManager.importSession(targetWorkspaceId, bundle as import('@pacman/shared/sessions').SessionBundle, mode)
  }
  ...
  server.handle(RPC_CHANNELS.sessions.IMPORT_REMOTE_TRANSFER, async (_ctx, targetWorkspaceId: string, payload: import('@pacman/shared/protocol').RemoteSessionTransferPayload) => {
    await sessionManager.waitForInit()
    if (!targetWorkspaceId || typeof targetWorkspaceId !== 'string') throw new Error('targetWorkspaceId is required')
