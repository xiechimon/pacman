...
import { getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace, updateWorkspaceRemoteServer } from '@pacman/shared/config'
import { perf } from '@pacman/shared/utils'
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspaces.CHECK_SLUG, async (_ctx, slug: string) => {
    const defaultWorkspacesDir = join(homedir(), '.pacman', 'workspaces')
  ...
  server.handle(RPC_CHANNELS.theme.GET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string) => {
    const { getWorkspaces } = await import('@pacman/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@pacman/shared/workspaces/storage')
  ...
  server.handle(RPC_CHANNELS.theme.SET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@pacman/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@pacman/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
  ...
  server.handle(RPC_CHANNELS.views.LIST, async (_ctx, workspaceId: string) => {
    ...
    const { listViews } = await import('@pacman/shared/views/storage')
    return listViews(workspace.rootPath)
  })

  // Save views (replaces full array)
  server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId: string, views: import('@pacman/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { saveViews } = await import('@pacman/shared/views/storage')
  ...
  server.handle(RPC_CHANNELS.toolIcons.GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@pacman/shared/config/storage')
    const { loadToolIconConfig } = await import('@pacman/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@pacman/shared/utils/icon-encoder')
  ...
  server.handle(RPC_CHANNELS.logo.GET_URL, async (_ctx, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@pacman/shared/utils/logo')
