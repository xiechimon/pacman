...
import type { PageActionRequest } from '@pacman/shared/pages'
import type { PageActionBroker, PageActionExecutors } from '@pacman/shared/pages'
...
export function registerPagesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  const mcpPools = new Map<string, import('@pacman/shared/mcp').McpClientPool>()
  ...
  function buildApiExecutor(workspaceRootPath: string): NonNullable<PageActionExecutors['executeApi']> {
    ...
    let refreshManager: import('@pacman/shared/sources').TokenRefreshManager | undefined
    return async (invocation, { signal }) => {
      const {
        ...
        createTokenGetter,
        executeApiRequest,
      } = await import('@pacman/shared/sources')
      ...
      let credentialSource: import('@pacman/shared/sources').ApiCredentialSource
  ...
  async function getBroker(workspaceId: string, workspaceRootPath: string): Promise<PageActionBroker> {
    ...
    const { PageActionBroker } = await import('@pacman/shared/pages')
    const { loadWorkspaceSources } = await import('@pacman/shared/sources')
    ...
    const { McpClientPool } = await import('@pacman/shared/mcp')
  ...
  server.handle(RPC_CHANNELS.pages.GET_ONE, async (_ctx, workspaceId: string, pageIdOrSlug: string) => {
    ...
    const { loadPage, loadPageById } = await import('@pacman/shared/pages')
    return loadPage(workspace.rootPath, pageIdOrSlug)
      ?? loadPageById(workspace.rootPath, pageIdOrSlug)
  })

  // Create a new page
  server.handle(RPC_CHANNELS.pages.CREATE, async (_ctx, workspaceId: string, input: import('@pacman/shared/pages').CreatePageInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createPage } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.UPDATE, async (
    ...
    workspaceId: string,
    pageSlug: string,
    patch: import('@pacman/shared/pages').UpdatePagePatch,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { updatePage } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.DELETE, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deletePageWithUnpublish } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.GET_CONTENT, async (_ctx, workspaceId: string, pageSlug: string) => {
    ...
    const { loadPageContent, loadPageConfig } = await import('@pacman/shared/pages')
    return {
  ...
  server.handle(RPC_CHANNELS.pages.SET_CONTENT, async (_ctx, workspaceId: string, pageSlug: string, content: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { savePageContent } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.GET_DATA, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) return null
    const { readPageDataSnapshot } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.LIST_GRANTS, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) return []
    const { loadPageConfig } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.ISSUE_GRANT, async (
    ...
    workspaceId: string,
    pageSlug: string,
    input: import('@pacman/shared/pages').AddPageGrantInput,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { addPageGrant } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.REVOKE_GRANT, async (_ctx, workspaceId: string, pageSlug: string, grantId: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { revokePageGrant } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.CREATE_LEASE, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { loadPageContent, computePageContentDigest } = await import('@pacman/shared/pages')
  ...
  async function buildPublisher() {
    const { PagePublisher, createCredentialPagePublishTokenStore } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.GET_SHARE_CAPABILITIES, async () => {
    const { isPagesSharingEnabled } = await import('@pacman/shared/feature-flags')
  ...
  server.handle(RPC_CHANNELS.pages.GET_SHARE_DATA_SCAN, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { scanPageShareData } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.pages.GET_THUMBNAIL, async (_ctx, workspaceId: string, pageSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) return null
    const { loadPageConfig, getPageThumbnailPath, isThumbnailFresh } = await import('@pacman/shared/pages')
