...
export function registerProjectsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  async function broadcastChanged(workspaceId: string, workspaceRootPath: string): Promise<void> {
    const { loadWorkspaceProjects } = await import('@pacman/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.GET_ONE, async (_ctx, workspaceId: string, projectIdOrSlug: string) => {
    ...
    const { loadProject, loadProjectById } = await import('@pacman/shared/projects')
    return loadProject(workspace.rootPath, projectIdOrSlug)
      ?? loadProjectById(workspace.rootPath, projectIdOrSlug)
  })

  // Create a new project
  server.handle(RPC_CHANNELS.projects.CREATE, async (_ctx, workspaceId: string, input: import('@pacman/shared/projects').CreateProjectInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createProject } = await import('@pacman/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.UPDATE, async (
    ...
    workspaceId: string,
    projectSlug: string,
    patch: Partial<Omit<import('@pacman/shared/projects').ProjectConfig, 'id' | 'slug' | 'createdAt'>>,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { updateProject } = await import('@pacman/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.DELETE, async (_ctx, workspaceId: string, projectSlug: string) => {
    ...
    const { loadProject, deleteProject } = await import('@pacman/shared/projects')
    ...
    const { unbindProjectFromSessions } = await import('@pacman/shared/sessions')
    const touched = await unbindProjectFromSessions(workspace.rootPath, project.config.id)
    const { unbindProjectFromPages } = await import('@pacman/shared/pages')
  ...
  server.handle(RPC_CHANNELS.projects.LIST_ASSETS, async (_ctx, workspaceId: string, projectSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) return []
    const { listProjectAssets } = await import('@pacman/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.UPLOAD_ASSET, async (
    ...
    workspaceId: string,
    projectSlug: string,
    input: import('@pacman/shared/projects').UploadProjectAssetInput,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { uploadProjectAsset } = await import('@pacman/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.DELETE_ASSET, async (
    ...
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteProjectAsset } = await import('@pacman/shared/projects')
