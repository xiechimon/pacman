import { RPC_CHANNELS, type LlmConnectionSetup } from '@pacman/shared/protocol'
import { getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, isAnthropicProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, toBedrockNativeId, deriveBedrockRegionPrefix } from '@pacman/shared/config'
import { getCredentialManager } from '@pacman/shared/credentials'
import { setSetupDeferred } from '@pacman/shared/config/storage'
import {
  resolveSetupTestConnectionHint,
  testBackendConnection,
  validateStoredBackendConnection,
} from '@pacman/shared/agent/backend'
import { getModelRefreshService } from '@pacman/server-core/model-fetchers'
import { parseTestConnectionError, createBuiltInConnection, validateModelList, piAuthProviderDisplayName, validateSetupTestInput, setupTestRequiresApiKey, resolveCustomEndpointSetup } from '@pacman/server-core/domain'
import { getWorkspaceOrThrow, buildBackendHostRuntimeContext } from '@pacman/server-core/handlers'
import { pushTyped, type RpcServer } from '@pacman/server-core/transport'
import type { HandlerDeps } from '../handler-deps'
import { randomUUID } from 'node:crypto'
import { CLIENT_OPEN_EXTERNAL } from '@pacman/server-core/transport'
...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.settings.SETUP_LLM_CONNECTION, async (_ctx, setup: LlmConnectionSetup): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (setup.piAuthProvider && !isCustomEndpointCompat) {
        ...
        // Update connection name to show the actual provider (e.g. "Pacman Backend (Google AI Studio)")
        const providerName = piAuthProviderDisplayName(setup.piAuthProvider)
        if (providerName) {
          updates.name = `Pacman Backend (${providerName})`
        }
  ...
  server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params: import('@pacman/shared/protocol').TestLlmConnectionParams): Promise<import('@pacman/shared/protocol').TestLlmConnectionResult> => {
  ...
  server.handle(RPC_CHANNELS.pi.GET_API_KEY_PROVIDERS, async () => {
    const { getPiApiKeyProviders } = await import('@pacman/shared/config')
    return getPiApiKeyProviders()
  })

  server.handle(RPC_CHANNELS.pi.GET_PROVIDER_BASE_URL, async (_ctx, provider: string) => {
    const { getPiProviderBaseUrl } = await import('@pacman/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.TEST, async (_ctx, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { parseValidationError } = await import('@pacman/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.SET_WORKSPACE_DEFAULT, async (_ctx, workspaceId: string, slug: string | null): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { loadWorkspaceConfig, saveWorkspaceConfig } = await import('@pacman/shared/workspaces')
  ...
  server.handle(RPC_CHANNELS.chatgpt.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    const { prepareChatGptOAuth } = await import('@pacman/shared/auth')
  ...
  server.handle(RPC_CHANNELS.chatgpt.COMPLETE_OAUTH, async (ctx, args: {
    ...
    try {
      const { exchangeChatGptTokens } = await import('@pacman/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  // Start GitHub Copilot OAuth flow (device flow via Pi SDK).
  //
  // Device flow + token exchange can take 30-60s on a real GitHub account. The
  // RPC layer enforces REQUEST_TIMEOUT_MS = 30_000 (see
  // packages/shared/src/protocol/types.ts:158), so we cannot await the full
  // flow here without tripping the timeout in the renderer. Instead:
  //   1. Kick off `loginGitHubCopilot` in the background.
  //   2. Return { success: true, pending: true } as soon as the device code
  //      is surfaced (well under 5s typical).
  //   3. When the background flow settles, push `copilot:authResult` with the
  //      terminal success/error — the renderer awaits this push before calling
  //      saveAndValidateConnection.
  server.handle(RPC_CHANNELS.copilot.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    try {
      const { loginGitHubCopilot } = await import('@pacman/shared/auth')
      const credentialManager = getCredentialManager()

      // Cancel any previous in-flight flow
      copilotOAuthAbort?.abort()
      copilotOAuthAbort = new AbortController()
      const signal = copilotOAuthAbort.signal
      const clientId = ctx.clientId

      deps.platform.logger?.info(`Starting GitHub Copilot OAuth device flow for connection: ${connectionSlug}`)

      const flow = loginGitHubCopilot({
        onDeviceCode: ({ userCode, verificationUri }) => {
          deps.platform.logger?.info(`[GitHub OAuth] Device code: ${userCode}`)
          pushTyped(server, RPC_CHANNELS.copilot.DEVICE_CODE, { to: 'client', clientId }, {
            userCode,
            verificationUri,
          })
          server.invokeClient(clientId, CLIENT_OPEN_EXTERNAL, verificationUri).catch(err => {
        ...
        .then(async credentials => {
          ...
          pushTyped(server, RPC_CHANNELS.copilot.AUTH_RESULT, { to: 'client', clientId }, { success: true })
        })
        .catch(error => {
          if (!signal.aborted) {
            const message = error instanceof Error ? error.message : 'OAuth authentication failed'
            deps.platform.logger?.error('GitHub Copilot OAuth failed:', error)
            pushTyped(server, RPC_CHANNELS.copilot.AUTH_RESULT, { to: 'client', clientId }, {
              success: false,
              error: message,
            })
          }
        ...
        .finally(() => {
          if (copilotOAuthAbort?.signal === signal) copilotOAuthAbort = null
        })
      ...
      deps.platform.logger?.error('GitHub Copilot OAuth failed to start:', error)
      return {
        success: false,
