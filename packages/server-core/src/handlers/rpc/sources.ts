...
import { loadWorkspaceSources } from '@pacman/shared/sources'
import { safeJsonParse } from '@pacman/shared/utils/files'
...
export function registerSourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Create a new source
  server.handle(RPC_CHANNELS.sources.CREATE, async (_ctx, workspaceId: string, config: Partial<import('@pacman/shared/sources').CreateSourceInput>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createSource } = await import('@pacman/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.DELETE, async (_ctx, workspaceId: string, sourceSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteSource } = await import('@pacman/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.SAVE_CREDENTIALS, async (_ctx, workspaceId: string, sourceSlug: string, credential: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { loadSource, getSourceCredentialManager, markSourceAuthenticated } = await import('@pacman/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.GET_PERMISSIONS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    const { getSourcePermissionsPath } = await import('@pacman/shared/agent')
  ...
  server.handle(RPC_CHANNELS.workspace.GET_PERMISSIONS, async (_ctx, workspaceId: string) => {
    ...
    const { existsSync, readFileSync } = await import('fs')
    const { getWorkspacePermissionsPath } = await import('@pacman/shared/agent')
  ...
  // Get default permissions from ~/.craft/permissions/default.json
  server.handle(RPC_CHANNELS.permissions.GET_DEFAULTS, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@pacman/shared/agent')
  ...
  server.handle(RPC_CHANNELS.sources.GET_MCP_TOOLS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    try {
      ...
      const { CraftMcpClient } = await import('@pacman/shared/mcp')
      ...
      const { loadSourcePermissionsConfig, permissionsConfigCache } = await import('@pacman/shared/agent')
