...
import { getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath } from '@pacman/shared/config'
import { classifyExternalUrl, formatBlockedUrlError } from '@pacman/shared/utils/url-safety'
import { isUsableGitBashPath, validateGitBashPath } from '@pacman/server-core/services'
...
} from '@pacman/server-core/transport'
...
function parseInternalPacmanDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'pacman:') return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle pacman:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalPacmanDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing pacman:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for pacman:// URL')
