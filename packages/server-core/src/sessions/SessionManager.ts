import type { EventSink, RpcServer } from '@pacman/server-core/transport'
import { CLIENT_BROWSER_INVOKE } from '@pacman/server-core/transport'
import type { ISessionManager, IBrowserPaneManager, ExecutePromptAutomationInput } from '@pacman/server-core/handlers'
import { RemoteBrowserPaneManager } from './RemoteBrowserPaneManager'
import { validateFilePath, getWorkspaceAllowedDirs } from '@pacman/server-core/handlers'
import { createScopedLogger, CONSOLE_LOGGER, type PlatformServices, type Logger } from '@pacman/server-core/runtime'
import { basename, dirname, join } from 'path'
import { existsSync } from 'fs'
import { readFile, writeFile, mkdir } from 'fs/promises'
import { randomUUID } from 'node:crypto'
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary, resolveKeepBackgroundTasksAlive } from '@pacman/shared/agent'
import {
  ...
  type PostInitResult,
} from '@pacman/shared/agent/backend'
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, resetManagedAnthropicAuthEnvVars, resolveMidStreamBehavior, getPersistedUiLanguage, resolveTitleLanguageName } from '@pacman/shared/config'
import type { MidStreamBehavior } from '@pacman/shared/config'
import { PrivilegedExecutionBroker } from '@pacman/server-core/services'
import { isValidWorkingDirectory } from '../utils/path-validation'
import { InitGate } from '@pacman/server-core/domain'
import { i18n } from '@pacman/shared/i18n'
import {
  getWorkspaces,
...
import type { ActiveSessionInfo, SessionProcessingStatus } from '@pacman/core/types'
import { loadWorkspaceConfig } from '@pacman/shared/workspaces'
import {
  ...
  type SessionHeader,
  pickSessionFields,
} from '@pacman/shared/sessions'
import { loadWorkspaceSources, loadAllSources, getSourcesBySlugs, isSourceUsable, type LoadedSource, type McpServerConfig, getSourcesNeedingAuth, getSourceCredentialManager, TokenRefreshManager } from '@pacman/shared/sources'
import { listTaskSlugs, parseTaskSpec, uniqueTaskSlug } from '@pacman/shared/tasks'
...
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@pacman/shared/config'
import { getValidClaudeOAuthToken } from '@pacman/shared/auth'
import { resolveAuthEnvVars } from '@pacman/shared/config'
import { toolMetadataStore, getLastApiError } from '@pacman/shared/interceptor'
import { isParentTaskTool } from '@pacman/shared/utils/toolNames'
import { restoreFiles } from '@pacman/shared/utils/bundle-files'
import { getCredentialManager } from '@pacman/shared/credentials'
import { CraftMcpClient, McpClientPool, McpPoolServer } from '@pacman/shared/mcp'
import { type Session, type SessionEvent, type FileAttachment, type SendMessageOptions, type UnreadSummary, type RemoteSessionTransferPayload, type ImportRemoteSessionTransferResult, RPC_CHANNELS, generateMessageId } from '@pacman/shared/protocol'
import { messageToStored, storedToMessage, type Message, type StoredAttachment, type ToolDisplayMeta, type TokenUsage } from '@pacman/core/types'
import { formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrlAsync, getEmojiIcon, resetSummarizationClient, resolveToolIcon, readFileAttachment, selectSpreadMessages, normalizePath } from '@pacman/shared/utils'
import { loadAllSkills, loadSkillBySlug, invalidateSkillsCache, type LoadedSkill } from '@pacman/shared/skills'
import { invalidateContextFileCache } from '@pacman/shared/prompts/system'
import { getToolIconsDir, getMiniModel } from '@pacman/shared/config'
import { getDefaultSummarizationModel } from '@pacman/shared/config/models'
import type { SummarizeCallback } from '@pacman/shared/sources'
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL, normalizeThinkingLevel } from '@pacman/shared/agent/thinking-levels'
import { evaluateAutoLabels } from '@pacman/shared/labels/auto'
import { listLabels, loadLabelConfig } from '@pacman/shared/labels/storage'
import { extractLabelId, resolveSessionLabels, findTaskItemLabelId } from '@pacman/shared/labels'
import { ensureLabelsExist, ensureTaskItemLabel } from '@pacman/shared/labels/crud'
import { loadStatusConfig } from '@pacman/shared/statuses/storage'
import { AutomationSystem, createPromptHistoryEntry, appendAutomationHistoryEntry, type AutomationSystemMetadataSnapshot } from '@pacman/shared/automations'
import { buildBackendRuntimeSignature, buildRestartRequiredSignature, filterAttachmentsForModelInput } from './runtime-config'
import { validateArchiveTarget } from './archive-guards'

// Import from server-core domain utilities
import { sanitizeForTitle, shouldActivateBrowserOverlay, normalizeBrowserToolName, rollbackFailedBranchCreation, releaseBrowserOwnershipOnForcedStop } from '@pacman/server-core/domain'
import { resizeImageForAPI, resizeIconBuffer } from '@pacman/server-core/services'
...
// validateSpawnAttachmentPath removed — use shared validateFilePath from @pacman/server-core/handlers
 ...
 * what the LLM sees. See pacmans-oss#782.
 */
...
async function applyBridgeUpdates(
  ...
  mcpServers: Record<string, import('@pacman/shared/agent/backend').SdkMcpServerConfig>,
  sessionId: string,
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.craft/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.craft/tool-icons/
...
interface ManagedSession {
  ...
  // pendingTextParent) has been removed. Pacman now provides parentToolUseId
   ...
   * so the SDK's session-manager has updated its leaf — see pacmans-oss#782).
   * Capped at PI_SDK_MESSAGE_ID_CACHE_LIMIT to bound memory in long sessions.
   */
  piSdkMessageToCraftMessage?: Map<string, string>
  // Source-activation auto-retry (pacmans-oss#804). When a source activates
...
export class SessionManager implements ISessionManager {
  ...
  private pendingCredentialResolvers: Map<string, (response: import('@pacman/shared/protocol').CredentialResponse) => void> = new Map()
  ...
  /**
   * WS2 keep-alive flag (default ON, opt-out via `PACMAN_KEEP_BG_AGENTS_ALIVE=0`).
  ...
  setupConfigWatcher(workspaceRootPath: string, workspaceId: string): void {
    ...
    const callbacks: ConfigWatcherCallbacks = {
      ...
      onSkillChange: async (slug, skill) => {
        ...
        const { loadAllSkills } = await import('@pacman/shared/skills')
        const skills = loadAllSkills(workspaceRootPath)
        this.broadcastSkillsChanged(workspaceId, skills)
      },
  ...
  private broadcastAppThemeChanged(theme: import('@pacman/shared/config').ThemeOverrides | null): void {
    if (!this.eventSink) return
  ...
  private broadcastSkillsChanged(workspaceId: string, skills: import('@pacman/shared/skills').LoadedSkill[]): void {
    if (!this.eventSink) return
    sessionLog.info(`Broadcasting skills changed (${skills.length} skills)`)
    this.eventSink(RPC_CHANNELS.skills.CHANGED, { to: 'workspace', workspaceId }, workspaceId, skills)
  }

  private broadcastPagesChanged(workspaceId: string, pages: import('@pacman/shared/pages').LoadedPage[]): void {
    if (!this.eventSink) return
   ...
   * See: https://github.com/lukilabs/pacmans-oss/issues/39
   */
  /**
   * Reinitialize authentication environment variables.
   *
  ...
  async handleCredentialInput(
    sessionId: string,
    requestId: string,
    response: import('@pacman/shared/protocol').CredentialResponse
  ): Promise<void> {
    const managed = this.sessions.get(sessionId)
    ...
    try {
      ...
      const { markSourceAuthenticated } = await import('@pacman/shared/sources')
  ...
  async createSession(
    workspaceId: string,
    options?: import('@pacman/shared/protocol').CreateSessionOptions,
    ...
    if (requestedProjectId) {
      const { loadProjectById } = await import('@pacman/shared/projects')
    ...
    if (validatedBranch) {
      ...
      // full-history fork — see pacmans-oss#782.
      if (
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    ...
    if (!managed.agent) {
      ...
      // The SDK subprocess reads PACMAN_SESSION_DIR to write tool-metadata.json;
      ...
      process.env.PACMAN_SESSION_DIR = sessionDirForMetadata
      ...
      const envOverrides: Record<string, string> = {
        PACMAN_WORKSPACE_PATH: managed.workspace.rootPath,
      ...
      managed.agent = createBackendFromResolvedContext({
        ...
        enable1MContext: await (async () => { const { getEnable1MContext } = await import('@pacman/shared/config/storage'); return getEnable1MContext(); })(),
      ...
      // The legacy onCredentialRequest callback has been removed from Pacman
      ...
      mergeSessionScopedToolCallbacks(managed.id, {
        ...
        // which must stay dependency-free of @pacman/shared); the creation flow
        ...
        pages: buildPagesToolCallbacks({
          ...
          onPagesMutated: async (pageSlug: string) => {
            ...
            const { loadWorkspacePages } = await import('@pacman/shared/pages')
        ...
        activateSourceInSessionFn: async (sourceSlug: string) => {
          ...
          // landing in a fresh turn with tools live (pacmans-oss#804).
  ...
  async setSessionConnection(sessionId: string, connectionSlug: string): Promise<void> {
    ...
    const { getLlmConnection } = await import('@pacman/shared/config/storage')
   ...
   */
  async shareToViewer(sessionId: string): Promise<import('@pacman/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
    ...
    try {
      ...
      const { VIEWER_URL } = await import('@pacman/shared/branding')
   ...
   */
  async updateShare(sessionId: string): Promise<import('@pacman/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
   ...
   */
  async revokeShare(sessionId: string): Promise<import('@pacman/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    // Cancel any pending source-activation auto-retry timer (pacmans-oss#804).
  ...
  async sendMessage(
    ...
    // Source-activation auto-retry dedup (pacmans-oss#804). When the server
  ...
  respondToPermission(
    ...
    allowed: boolean,
    alwaysAllow: boolean,
    options?: import('@pacman/shared/protocol').PermissionResponseOptions,
  ): boolean {
    const managed = this.sessions.get(sessionId)
    if (managed?.agent) {
   ...
   */
  async respondToCredential(sessionId: string, requestId: string, response: import('@pacman/shared/protocol').CredentialResponse): Promise<boolean> {
  ...
  private async processEvent(managed: ManagedSession, event: AgentEvent): Promise<void> {
    ...
    switch (event.type) {
      ...
      case 'tool_start': {
        ...
        // Use parentToolUseId directly from the event — Pacman resolves this
      ...
      case 'tool_result': {
        // toolName comes directly from Pacman (resolved via ToolIndex)
        ...
        // parentToolUseId comes from Pacman (SDK-authoritative) or existing message
      ...
      case 'complete':
        // Complete event from Pacman - accumulate usage from this turn
  ...
  async executePromptAutomation(
    ...
    // client timeout (pacmans-oss#943). The session streams live either
  ...
  private async generateRemoteTransferSummary(managed: ManagedSession): Promise<string | null> {
    ...
    const envOverrides: Record<string, string> = {
      PACMAN_WORKSPACE_PATH: workspaceRootPath,
      ...(miniModel ? { ANTHROPIC_DEFAULT_HAIKU_MODEL: miniModel } : {}),
    }
