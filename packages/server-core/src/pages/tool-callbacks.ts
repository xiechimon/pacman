 ...
 * (@pacman/shared/pages) — including deletePageWithUnpublish, shared
...
import type { LoadedPage, PageConfig, PageDataSnapshot, PageKind, PageRefreshSpec, UpdatePagePatch } from '@pacman/shared/pages'
...
export function buildPagesToolCallbacks(deps: PagesToolCallbacksDeps): PagesToolCallbacks {
  ...
  async function loadDetails(slug: string, includeContent = false): Promise<PageToolDetails | null> {
    const { loadPage, loadPageById, readPageDataSnapshot, loadPageContent } = await import('@pacman/shared/pages')
  ...
  return {
    ...
    async updatePage(slug: string, patch: UpdatePageToolPatch): Promise<PageToolDetails> {
      const { updatePage, savePageContent, loadPage } = await import('@pacman/shared/pages')
    ...
    async writePageData(slug: string, patch: PageDataToolPatch) {
      const { writePageData } = await import('@pacman/shared/pages')
    ...
    async deletePage(slug: string) {
      const { deletePageWithUnpublish, loadPage } = await import('@pacman/shared/pages')
      const existing = loadPage(workspaceRootPath, slug)
      if (!existing) throw new Error(`Page not found: ${slug}`)
