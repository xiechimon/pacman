...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.PACMAN_DEBUG === 'true' || process.env.PACMAN_IS_PACKAGED !== 'true') {
 ...
 * - PACMAN_APP_ROOT — override appRootPath (default: cwd)
 * - PACMAN_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - PACMAN_IS_PACKAGED — 'true' for production (default: false)
 * - PACMAN_VERSION — app version string (default: '0.0.0-dev')
 * - PACMAN_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.PACMAN_DEBUG === 'true' || process.env.PACMAN_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.PACMAN_APP_ROOT || process.cwd(),
    resourcesPath: process.env.PACMAN_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.PACMAN_IS_PACKAGED === 'true',
    appVersion: process.env.PACMAN_VERSION || options?.appVersion || '0.0.0-dev',
